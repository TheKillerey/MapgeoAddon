...
def import_materials_from_file(filepath: str, create_textures: bool = True) -> Dict[str, bpy.types.Material]:
    ...
    # Store other_entries and entry_order in Blender Text data block for preservation
...
def _store_other_entries(other_entries: dict, entry_order: list, source_filepath: str):
    """Store non-material entries and entry order in Blender Text data block for later export"""
    import pickle
    import base64
    
    # Serialize both other_entries and entry_order
    try:
        store_data = {
            'other_entries': other_entries,
            'entry_order': entry_order,
        }
        serialized = base64.b64encode(pickle.dumps(store_data)).decode('ascii')
        
        # Store in Text data block
        ...
        text_block.write(f"# DATA:\n")
        text_block.write(serialized)
        
        print(f"Stored {len(other_entries)} other entries + {len(entry_order)} order entries in Blender text block '{text_name}'")
    except Exception as e:
